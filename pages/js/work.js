...
(function IndexInit() {
	...
	const eventFunctions = {
		run: (ev, elem) => {
			...
			const command = commands[dataCommand] || dataCommand;
			const path = paths[dataPath] || dataPath;
	...
	function handleGetProjectTree(ev) {
		...
		const addProjectCollectionToDom = (programs, projectPath) => {
			...
			programs.forEach(program => {
				...
				program.sort((a, b) => {
					const aBranch = a.splitPath[branchNameIndex].toLowerCase();
					const bBranch = b.splitPath[branchNameIndex].toLowerCase();

					if (aBranch === 'main' || aBranch === 'demo'){
						return -1;
					}else if(bBranch === 'main' || bBranch === 'demo'){
						return 1;
					}
				...
				program.forEach(programBranch => {
					...
					const lBranchName = branchName.toLowerCase();
					
					const btnClass = (lBranchName === 'main' || lBranchName === 'demo') ? 'btn-primary' : 'btn-info';
					newContent += `<div class="btn-group mb-1">
							<button data-function="click:run" data-command="vs" data-path="${programBranch.fullPath}" title="${programBranch.fullPath}"
							 class="btn ${btnClass} btn-sm border-dark">${branchName}</button>
							<button data-function="click:run" data-command="code" data-path="${programBranch.pathToFile}" title="${programBranch.pathToFile}"
							 class="btn ${btnClass} btn-sm border-dark"><i class="fas fa-code"></i></button>
							<button data-function="click:run" data-command="explorer" data-path="${programBranch.pathToFile}" title="${programBranch.pathToFile}"
							 class="btn ${btnClass} btn-sm border-dark"><i class="fas fa-folder-open"></i></button>
						</div>\n`;
				});
				newContent += '</div>\n';
			});
		...
		unbranched.forEach(file => {
			...
			<button data-function="click:run" data-command="vs" data-path="${file.fullPath}" title="${file.fullPath}"
			 class="btn btn-secondary btn-sm border-dark">${name}</button>
			<button data-function="click:run" data-command="explorer" data-path="${file.pathToFile}" title="${file.fullPath}"
